...
public interface MyProcedureRepository extends JpaRepository<MyProcedure, Long> {
    ...
    @Query("SELECT new com.wetech.backend_spring_wetech.dto.procedure.MyProcedureResultDTO(p.procedureId, p.serviceType, p.serviceTypeTitle, p.typeCompany, p.typeCompanyTitle, p.code, mp.submissionCount, mp.submissionDate, mp.taxAuthority, mp.status) " +
